...
const counts = ref({ people: 0, unassigned: 0, suggestions: 0 });

async function refreshCounts() {
  try {
    const [stats, list, faces] = await Promise.all([
    ...
    // unassigned count: get from stats (total - clustered)
    if (stats) counts.value.unassigned = Math.max(0, stats.faces_total - stats.faces_clustered);
    // Touch faces to keep API alive / avoid unused var
    void faces;
  } catch {
    /* silent */
  }
